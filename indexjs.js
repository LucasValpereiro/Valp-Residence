...
function buildThread(){
  ...
  // no mobile o fio corre junto à borda esquerda com leve ondulação; no desktop faz zigue-zague amplo
  const left  = mob?0.055:0.052;
  const right = mob?0.16:0.948;
  const start = mob?0.08:0.5;
  const pts=[{x:W*start,y:0}];
  secs.forEach(s=>{
  ...
  pts.push({x:W*(mob?0.08:0.5),y:H-40});
