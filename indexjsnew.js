...
function enviarWA(e){
  ...
  if(btn){const o=btn.textContent;btn.textContent='✅ Enviando...';btn.disabled=true;setTimeout(()=>{btn.textContent=o;btn.disabled=false;},3000);}
  salvarNoFirebase({nome:nome||'',telefone:tel||'',email:email||'',apartamento:apt||'',checkin:ci?fmtData(ci):'',checkout:co?fmtData(co):'',hospedes:g||'',mensagem:msg||'',canal:'site',formulario:'contato'});
}
...
function abrirModal(id){
  ...
  if(a.rooms&&a.rooms.length){
    ...
    rw.innerHTML=a.rooms.map(r=>{
      let fi=mPhotos.findIndex(p=>p.label===r.name);
      // sofá-cama normalmente fica na sala: usa foto da Sala de Estar como fallback
      const ehSofa=/sof[áa]/i.test(r.beds||'');
      if(fi<0 && ehSofa) fi=mPhotos.findIndex(p=>p.label==='Sala de Estar');
      if(fi<0 && ehSofa) fi=mPhotos.findIndex(p=>p.label==='Sala de Jantar');
      const ph=fi>=0?`<div class="m-rph" style="background-image:url(${mPhotos[fi].src})" onclick="abrirLightbox(${fi})" title="Ver foto"></div>`:'';
...
function buildThread(){
  if(reduced){thread.style.display='none';return;}
  ...
  // zigue-zague amplo de um lado ao outro, em qualquer largura
  const left  = mob?0.10:0.052;
  const right = mob?0.90:0.948;
  let pts=[{x:W*0.5,y:0}];
  secs.forEach(s=>{
    const r=s.getBoundingClientRect(), top=r.top+window.scrollY;
    const side=s.dataset.node==='left'?left:right;
  ...
  tNodes.innerHTML=pts.slice(1,-1).map(p=>`<circle class="t-node" r="${mob?3:4}" cx="${p.x}" cy="${p.y}"/>`).join('');
  updateThread();
}
...
function reservarUnidade(){
  ...
  window.open(`https://wa.me/${WA}?text=${texto}`,'_blank');
  salvarNoFirebase({apartamento:mApt.name,checkin:ci?fmtData(ci):'',checkout:co?fmtData(co):'',hospedes:g||'',canal:'site',formulario:'unidade'});
}
